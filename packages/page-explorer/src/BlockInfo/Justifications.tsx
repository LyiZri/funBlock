...
function JustificationList({ value }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <Table empty={t<string>("No justifications available")} header={headerRef.current}>
      {justifications?.map((justification, index) => (
        <tr key={`justification:${index}`}>
          <td className="overflow">
            <Expander summary={justification[0].toString()}>{formatTuple(justification as unknown as Tuple)}</Expander>
          </td>
        </tr>
      ))}
    </Table>
